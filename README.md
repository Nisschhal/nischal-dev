...
Requires **Node 22.13+** (the `engines` floor), **pnpm 11.10** (pinned in
`packageManager`), and **Docker** for the container steps. Full setup in
...
A non-empty Markdown body is what earns the case-study page — an entry without one
falls back to a row.
